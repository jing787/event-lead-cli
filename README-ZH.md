...
> 日文版本: [README-JA.md](README-JA.md)
...
| 维度 | 默认权重 | 评估内容 | 依据示例 |
|------|----------|----------|----------|
| `company_fit` | 0.25 | 公司规模与行业是否符合 Dify 的目标客户画像 | "上市金融机构，规模与行业高度匹配目标客户" |
| `seniority_match` | 0.25 | 职位是否具备预算决策或采购影响力 | "副总层级，具备采购决策影响力" |
| `engagement_signal` | 0.35 | 现场的跟进意向积极程度 | "明确提出需要 demo，且项目时间较近" |
| `interest_alignment` | 0.15 | 需求方向与 Dify 核心能力的契合程度 | "关注流程自动化与知识库，与 Dify 能力直接匹配" |
...
通常是把注释行（以 `#` 开头）也一起粘贴进终端了。只执行真正的命令行。

**报错 `Error: Got unexpected extra argument (...)`**

`run_enrich.sh` 只需要一个配置路径参数，正确写法：
...
通常是早期失败运行留下了坏的 checkpoint（比如 API Key 配置异常），续跑时又读到了旧文件。删掉 checkpoint 后全量重跑：
...
通常是混用了旧版本生成的历史输出。先拉取最新代码，清理旧报告，再重跑：
...
首个请求可能受模型冷启动或网络波动影响，属于常见现象，不代表分组逻辑异常。长流程建议使用 `--resume`，避免重复计算已完成阶段。
