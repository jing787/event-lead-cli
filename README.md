...
```bash
git clone https://github.com/jing787/event-lead-cli.git
cd event-lead-cli
python3 -m venv .venv
...
|-----------|---------------|-----------------|----------------------|
| `company_fit` | 0.25 | Company size and industry match to target customer profile | "Public financial company; profile strongly matches the target ICP" |
| `seniority_match` | 0.25 | Budget authority or procurement influence | "VP-level contact with clear purchasing influence" |
| `engagement_signal` | 0.35 | On-site interest level and follow-up request | "Explicitly requested a demo with a near-term timeline" |
| `interest_alignment` | 0.15 | Alignment between stated interests and core capabilities | "Needs align with workflow automation and knowledge base use cases" |
...
Do not append a second config argument.
...
**Segments are not exactly A/B/C**
